...
class DataArguments:
    ...
    def __post_init__(self):
        ...
        if self.encode_in_path is not None:
            if os.path.isdir(self.encode_in_path):
                files = os.listdir(self.encode_in_path)
                self.encode_in_files = [
                    os.path.join(self.encode_in_path, f)
                    for f in files
                    if f.endswith('tsv') or f.endswith('json')
                ]
            elif os.path.isfile(self.encode_in_path):
                self.encode_in_files = [self.encode_in_path]
...
class DenseTrainingArguments(TrainingArguments):
    ...
    training_mode: str = field(default='oq.nll', metadata={"help": "training mode for DR"})

    # loss
    # temperature: float = field(default=1.0, metadata={"help": "The temperature in the negative log likelihood loss."})
    w1: float =  field(default=1.0, metadata={"help": "The weight on the negative log likelihood loss."})
    w2: float = field(default=1.0, metadata={"help": "The weight on the qp local ranking alignment loss."})
    w3: float = field(default=1.0, metadata={"help": "The weight on the pq local ranking alignment loss."})
