...
class DenseModel(nn.Module):
    ...
    def forward(
        ...
        elif self.train_args.training_mode == 'qv.nll':
            assert query_variation is not None
            q_hidden, q_reps = None, None
            p_hidden, p_reps = self.encode_passage(passage)
            qv_hidden, qv_reps = self.encode_query(query_variation)
        elif self.train_args.training_mode == 'oq.qv.lra':   ## local ranking alignment
            assert query_variation is not None
        ...
        if self.training:
            ...
            if self.train_args.training_mode == 'oq.nll':
                ...
                ### qv_nll
                qvp_scores = torch.matmul(qv_reps, p_reps.transpose(0, 1)).view(effective_bsz, -1)
                target = torch.arange(qvp_scores.size(0), device=qvp_scores.device, dtype=torch.long)
                target = target * self.data_args.train_n_passages
                loss = self.cross_entropy(qvp_scores, target)

            elif self.train_args.training_mode == 'oq.qv.lra':
                ### qv_nll
                qvp_scores = torch.matmul(qv_reps, p_reps.transpose(0, 1))
                target = torch.arange(qvp_scores.size(0), device=qvp_scores.device, dtype=torch.long)
                target = target * self.data_args.train_n_passages
                ...
                pqv_scores = torch.matmul(p_reps, qv_reps.transpose(0, 1))
