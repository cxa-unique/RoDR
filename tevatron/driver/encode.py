...
def main():
    ...
    # Path
    if data_args.encoded_save_path.endswith(".pt"):
        check_path = '/'.join(data_args.encoded_save_path.split('/')[:-1])
    else:
        check_path = data_args.encoded_save_path
    ...
    if data_args.encode_in_path:
        logger.info(f'Encode_in_path:{data_args.encode_in_path}')
        logger.info(f'Encode_in_files:{data_args.encode_in_files}')
        encode_dataset = EncodeDataset(data_args.encode_in_files, tokenizer, max_len=text_max_length)
