...
class SimpleTrainPreProcessor:
    ...
    def __post_init__(self):
        ...
        if self.query_variation_file != 'None':
            self.query_variations = self.read_queries(self.query_variation_file)
        else:
            self.query_variations = None
    ...
    def get_variation(self, q):
        query_variation_encoded = self.tokenizer.encode(
            self.query_variations[q],
            add_special_tokens=False,
            max_length=self.max_length,
            truncation=True
        )
        return query_variation_encoded
    ...
    def process_one(self, train):
        ...
        if self.query_variation_file != 'None':
            train_example = {
                ...
                'query_variation': self.get_variation(q),
