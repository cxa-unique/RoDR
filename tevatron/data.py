...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, item) -> [BatchEncoding, List[BatchEncoding]]:
        ...
        if 'query_variation' in group:
            encoded_query_variation = self.create_one_example(group['query_variation'], is_query=True)
        else:
            encoded_query_variation = None
        ...
        if encoded_query_variation is not None:
            return encoded_query, encoded_passages, encoded_query_variation
        else:
...
class QPCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        if len(features[0]) == 3:
            qv = [f[2] for f in features]
            if isinstance(qv[0], list):
                qv = sum(qv, [])
            qv_collated = self.tokenizer.pad(
                qv,
                padding='max_length',
                max_length=self.max_q_len,
                return_tensors="pt",
            )
            return q_collated, d_collated, qv_collated
        else:
